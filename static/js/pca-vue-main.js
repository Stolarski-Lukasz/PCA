...
const vue_app = Vue.createApp({
  ...
  methods: {
    search(event) {
      ...
      formData.append('user_search_field_value', user_search_field_value)
      formData.append('checked_string', checked_string)
      formData.append('database_indexes_string', database_indexes_string)
    ...
    pagination(event) {
      ...
      formData.append('database_indexes', database_indexes_string)
      formData.append('previous_search_start_index', previous_search_start_index)
    ...
    play(event) {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          console.log(resulting_audio_file)

          // automatic play
          var url = resulting_audio_file;
          Vue.nextTick(function () {
      ...
      var formData = new FormData();
      formData.append('audio_file_name', audio_file_name)
      formData.append('textunit_start', textunit_start)
      formData.append('textunit_duration', textunit_duration)
      formData.append('start_buffer', start_buffer)
      formData.append('end_buffer', end_buffer)
      xhr.open('POST', '/create_audio/', true);
      xhr.send(formData);
    ...
    download(event) {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          link.href = resulting_audio_file;
